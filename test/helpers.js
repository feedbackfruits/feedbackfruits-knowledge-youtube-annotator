...
const captionResponse = require('./support/captions-response');
...
test('getCaptions: it converts text to concepts', async t => {
  const result = await Helpers.getCaptions(videoURL);
  // console.log(JSON.stringify(result));
  // The url changes everytime, so this is a bit hacky
  return t.deepEqual(result.map(x => filterObj(x, 'u')), captionResponse.map(x => filterObj(x, 'u')));
});

test('getCaptionsForLanguage: it get the captions for a specific language', async t => {
  const result = await Helpers.getCaptionsForLanguage(videoURL, 'en');
  // console.log(JSON.stringify(result));
  // The id changes everytime, so this is a bit hacky
  return t.deepEqual(result, captions);
});

// test('parseCaptions: it parses captions', async t => {
//   const result = await Helpers.parseCaptions(captionResponse);
//   console.log(JSON.stringify(result))
//   return t.deepEqual(result, captionText);
// });
